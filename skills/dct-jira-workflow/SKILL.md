...
2. **재작업 시 기존 완료 댓글 이어붙이기**:
   - `jira_get_issue(fields="comment")` 로 댓글 목록 조회
   - 본문 첫 줄이 `✅ 작업 완료 요약` 인 **가장 최신** 댓글 식별
   - **있음** → `jira_edit_comment` 로 수정 (기존 원문 + `---` + `## 🔁 추가 작업 (YYYY-MM-DD, KST)` 섹션 + 이번 요약). **신규 댓글 생성 금지**
   - **없음** → `jira_add_comment` 로 아래 포맷의 **단일 댓글** 신규 등록
   - 표가 포함되면 `jira-adf-table` 스킬로 ADF 변환 후 업로드 (ADF 거부 시 curl fallback)
3. 완료 댓글 포맷 — 본문 상단(또는 이어붙임 섹션 최상단) `## PR` 섹션에 Step 1 의 URL(또는 `생성 안 함`/`이미 병합됨`) 포함 (team-workflow 규칙):
  ```
  ✅ 작업 완료 요약
