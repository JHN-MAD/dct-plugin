...
1. `EnterPlanMode` 활성화 (또는 세션 시작 시 plan mode 진입)
...
> plan mode 가 활성화되지 않은 세션이라면 사용자에게 "plan mode 로 전환 후 진행" 을 권장. 강제할 수 없으면 ultrathink 수준의 분석을 보장한 뒤 승인 단계를 반드시 거친다.
