---
name: dct-slack
description: 팀 Slack 메시지 전송 — 최초 전송은 Member/Channel ID 직접 입력, 이후 주소록에 저장된 이름으로도 전송 가능
argument-hint: <Member ID | Channel ID | 저장된 이름> <메시지 본문>
---

# /dct-slack — Slack 메시지 전송

봇 `@매도비` 를 통해 `madupteam.slack.com` 의 채널 또는 DM 으로 메시지를 전송한다. `dct-slack` 스킬에 실제 로직을 위임.
...
### 이후 전송 — 저장된 이름 사용
```
/dct-slack 정하늘 아아ㅏㅇ아아아아
/dct-slack general 배포 완료
/dct-slack data-consulting 스프린트 플래닝 시작합니다
```
